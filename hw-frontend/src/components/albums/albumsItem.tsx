...
const AlbumItem:React.FC<Props> = ({name,year,id,image,isPublished,deleting,publishing,publishFunction,deleteFunction}) => {
  ...
  if(user?.role === 'admin') {
    button = (<Grid container justifyContent={'space-around'}  alignItems={'center'}>
      {isPublished ? (<Button onClick={publishFunction} disabled={publishing ? publishing === id : false} type={'button'} color={'warning'} variant={'contained'}>Not publish</Button>) :
        <Button type={'button'} onClick={publishFunction} disabled={publishing ? publishing === id : false} color={'success'} variant={'contained'}>Publish</Button>}

      <Button type={'button'} disabled={deleting ? deleting === id : false} color={'error'} onClick={deleteFunction} variant={'contained'}>Delete</Button>
    </Grid>);
  }
  return (
